...
//			 fAbsThresh <= 30; fAbsThresh += 0.1) {
		float fAbsThresh = FACTOID_ABS_THRESH;
//		for (float lRelThresh = LIST_REL_THRESH;
//			 lRelThresh <= 0.3; lRelThresh += 0.001f) {
		float lRelThresh = LIST_REL_THRESH;
		
		int targetCount = 0; // break after some targets
		for (TRECTarget target : targets) {
			
			// break after some targets
			if (targetCount >= 1000) return;
			else targetCount++;
			...
			for (int i = 0; i < questions.length; i++) {
				...
				if ((type.equals("FACTOID") && factoidLog) ||
					...
					results = TREC13To16Parser.loadResults(qs, type,
														   inputLogFile);
